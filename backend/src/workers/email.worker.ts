  ...
  async (job: Job<EmailJobData>) => {
    ...
    try {
      ...
      let emailData: any = {
        ...
        subject: template.subject.replace('{customerName}', payload.customer.name),
      ...
      switch (eventType) {
        case 'ABANDONED_CART':
          emailData.data = {
            ...
            productName: payload.products?.[0]?.name || 'Produto',
            productPrice: payload.products?.[0]?.price || 'R$ 0,00',
        ...
        case 'PIX_EXPIRED':
          emailData.data = {
            ...
            expirationDate: payload.expiration_date || payload.bank_slip_expiration_date,
            pixDiscount: 10,
          };
          break;
